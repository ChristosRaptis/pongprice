"""
This module contains the Scraper class which is used to scrape product data from a website
"""
...
class Scraper:
    ...
    def update_database(self, product_data: dict, cursor) -> None:
        ...
        """
        cursor.execute(
            "SELECT EXISTS(SELECT 1 FROM products WHERE url=%s);",
            (product_data["url"],),
        )
    ...
    def replace_matches(self, match):
        """
        Returns a string where the matched groups are formatted as needed
        """
        if match.group(4):
            return (
                ...
                for script in scripts:
                    ...
                    if "offers" in data:
                        ...
                        try:
                            ...
                            product_data["url"] = product_url
                            product_data["product_name"] = re.sub(
                                pattern, self.replace_matches, data["name"]
                            )
                        ...
                        try:
                            ...
                            product_data["product_price"] = data["object"]["offers"][
                                "price"
                            ]
                        except:
            ...
            product_list = list(
                tqdm(
                    pool.map(
                        ...
                        [conn] * len(self.product_sitemaps),
                    ),
